...
def get_problem_results(problem_id: str):
    ...
    # TODO: FIX THIS PATH --- only for testing purposes
    p = "/home/gnvf500/Gricel-Documents/GithubGris/EfficientPlanAdaptation/assets/planningProblem/data_example/dataevochecker_run1/NSGAII/"
    #search in p file that ends in _FRONT
    for file in os.listdir(p):
        if file.endswith("_Front"):
            results_path = os.path.join(p, file)
            break    
    
    if not os.path.exists(results_path):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Results file not found. The process may not be complete or it may have failed."
        )
